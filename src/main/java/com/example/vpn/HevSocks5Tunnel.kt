...
object HevSocks5Tunnel {
    ...
    /**
     * Start the tunnel event loop. **Blocking** — runs the native loop on the
     * calling thread and returns only when the tunnel exits (after [stop]).
     *
     * The caller (XrayVpnService) is responsible for running this on a dedicated
     * background thread so that `Thread.join()` can reliably wait for the tunnel
     * to terminate before the TUN fd is closed.
     */
