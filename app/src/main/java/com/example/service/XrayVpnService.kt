...
class XrayVpnService : VpnService() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ...
            ACTION_START -> {
                ...
                // کارهای سنگین (نوشتن فایل + JNI) را خارج از ترد اصلی انجام بده
                // تا ANR رخ ندهد. isRunning یک AtomicBoolean است و از هر تردی امن است.
                Thread({ startVpn(config) }, "vpn-start").start()
                return START_NOT_STICKY
            }
            else -> {
    ...
    private fun startVpn(config: VpnConfig) {
        ...
        try {
            ...
            // اگر edgeIps تنظیم شده باشد، قبل از ساخت کانفیگ، لبه‌ها را با
            // آدرس اسکن و رتبه‌بندی می‌کنیم (سریع‌ترین‌ها نگه داشته می‌شوند).
    ...
    private fun stopVpn() {
        ...
        // join روی ترد جاری IllegalThreadStateException می‌دهد — پس صرفاً رد می‌شویم.
        try {
            val current = Thread.currentThread()
            if (tunnelThread != null && tunnelThread !== current) {
    ...
    /**
     * اگر `config.edgeIps` تنظیم شده باشد، لبه‌ها را با TCP connect روی
     * [VpnConfig.port] اسکن و رتبه‌بندی می‌کند و `keepTop` لبه‌ی سریع را برمی‌گرداند.
     *
     * - اگر هیچ لبه‌ای پاسخ ندهد، لیست خالی برمی‌گردد تا از مسیر عادی
     *   (outbound تک‌آدرس با [VpnConfig.address]) استفاده شود.
     * - اگر لبه‌ای تنظیم نشده باشد، خالی برمی‌گردد (رفتار عادی قبلی).
     *
     * فراخوانی باید روی ترد پس‌زمینه باشد (این متد بلاکینگ است).
     */
    private fun resolveEdgePool(config: VpnConfig): List<String> {
        val candidates = IpPoolManager.parseEdges(config)
        if (candidates.isEmpty()) {
            return emptyList()
        }

        Log.i(TAG, "Scanning ${candidates.size} edge IPs on port ${config.port} ...")
        val ranked = IpPoolManager.scanAndRankBlocking(
            candidates = candidates,
            port = config.port,
            keepTop = IpPoolManager.DEFAULT_KEEP_TOP,
            timeoutMs = IpPoolManager.DEFAULT_TIMEOUT_MS
        )

        if (ranked.isEmpty()) {
            Log.w(TAG, "All edge IPs unreachable — falling back to primary address ${config.address}")
            return emptyList()
        }

        Log.i(TAG, "Edge pool ready (${ranked.size} fastest): ${ranked.joinToString()}")
        return ranked
    }
}
