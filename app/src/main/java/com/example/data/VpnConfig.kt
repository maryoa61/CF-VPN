...
data class VpnConfig(
    ...
    // Edge-IP pool (comma-separated IPs). When set, the app probes each candidate
    // at connect time (keeps the fastest reaching the `edge-*` outbound pool); then
    // XrayConfigGenerator emits one outbound per IP plus a `random` load balancer,
    // so Xray rotates the *connection* IP for every new connection while keeping
    // the same SNI/Host — mitigates per-IP traffic-volume throttling.
